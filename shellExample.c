...
int subshell_depth = 1; /* To keep track of subshell depth */
...
void imtheparent(pid_t child_pid, int run_in_background)
{
    ...
    /* From the man page for wait, waitpid:
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        else if (!strcmp(exec_argv[0], "sub"))
        {
            ...
            if (pid_from_fork == 0)
            {
                subshell_depth++;     /* increment depth */
                shell_pid = getpid(); /* update shell pid for child */
                /* Sub shell should continue with the loop, no further action */
            }
            else
            {
