...
/* Maximum number of arguments parsed    */
/* VARIABLE SECTION */
enum
{
    STATE_SPACE,
...
void imtheparent(pid_t child_pid, int run_in_background)
{
    ...
    {
            fprintf(stderr,
                    "  Parent says 'run_in_background=1 ... so we're not waiting
for the child'\n");
            return;
    }
    // TO-DO P5.4
    ...
    fprintf(stderr,
            "  Parent says 'wait() returned so the child with pid=%d is
            finished.'\n", child_pid);
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
       ...
       * is an ampersand (indicating background execution).        */
        buffer[i] = '\0'; /* Terminate input, overwriting the '&' if it
            /* If no command was given (empty line) the Shell just prints the
prompt again */
        if (!exec_argc)
            continue;
        ...
        else if (!strcmp(exec_argv[0], "cd") && exec_argc > 1)
        {
            ...
            /* Alternative: try chdir inside a forked child: if(fork() ==
      if (chdir(exec_argv[1]))
        ...
        else
        {
            ...
            if (pid_from_fork < 0)
            {
                /* Error: fork() failed.  Unlikely, but possible (e.g.
